...
use cep_snfei::{apply_localization, generate_snfei, SnfeiResult};

use crate::{EntityIdentifiers, EntityRecord, EntityStatus, EntityStatusCode, Snfei};
...
///
/// ```rust
/// use cep_entity::builder::{build_entity, EntityBuilderInput, AttestationInput};
///
...
pub fn build_entity(input: EntityBuilderInput) -> CepResult<EntityBuildResult> {
    ...
    let snfei = Snfei::new(&snfei_result.snfei.value)
        .map_err(|e| CepError::InvalidIdentifier(e.to_string()))?;
...
fn build_attestation(input: &AttestationInput) -> CepResult<Attestation> {
    ...
    Ok(Attestation::new(
        ...
        String::new(), // No cryptographic proof for manual attestations
...
mod tests {
    ...
    fn test_deterministic_snfei() {
        ...
        assert_eq!(result1.snfei_result.snfei.value, result2.snfei_result.snfei.value);
    }
}